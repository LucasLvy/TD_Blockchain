...
contract Evaluator {
    ...
    function ex3_showIRepaidTokens() public {
        require(
            exerciceProgression[msg.sender][2],
    ...
    function ex4_showIWithdrewTokens() public {
        require(
            exerciceProgression[msg.sender][1],
    ...
    function ex9_performFlashLoan() public {
        // Check https://docs.aave.com/developers/guides/flash-loans

        // Reading initial aDai balance
        uint256 initialBalance = USDC.balanceOf(
            address(studentExercice[msg.sender])
        );
        ...
        // Read end balance
        uint256 endBalance = USDC.balanceOf(
            address(studentExercice[msg.sender])
        );
        ...
        // Verify that contract did borrow
        require(
            endBalance > amountToBorrow,
            "Your contract does not hold 1M dollars"
        );
